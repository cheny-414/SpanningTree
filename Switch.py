...
class Switch(StpSwitch):
    ...
    def process_message(self, message: Message):
        ...
        new_root_flag = False
        shorter_distance_flag = False
        smaller_switch_through_flag = False

        # determine whether to update root
        if message.root < self.root:
            new_root_flag = True
            self.root = message.root
        # determine whether to update distance if same root but distance is shorter
        elif message.root == self.root and message.distance + 1 < self.distance:
            shorter_distance_flag = True
        # determine tiebreaker
        elif message.root == self.root and message.distance + 1 == self.distance and \
                message.origin < self.switchThrough:
            smaller_switch_through_flag = True

        if new_root_flag or shorter_distance_flag or smaller_switch_through_flag:
        ...
        # determine whether to update activeLinks
        if message.pathThrough:
            if message.origin not in self.activeLinks:
                self.activeLinks.append(message.origin)
        else:
            if message.origin in self.activeLinks and message.origin != self.switchThrough:
        ...
        if new_root_flag or shorter_distance_flag or smaller_switch_through_flag:
            for destinationID in self.links:
                self.send_message(
                    Message(self.root, self.distance, self.switchID, destinationID, self.switchThrough == destinationID)
                )
    ...
    def generate_logstring(self):
        ...
        output = ""
        sorted_active_links = self.activeLinks
        sorted_active_links.sort()
        for activeLink in sorted_active_links:
            output += str(self.switchID) + " - " + str(activeLink) + ", "
        output = output[:-2]
        return output
